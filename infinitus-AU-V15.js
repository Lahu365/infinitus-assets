...
/* ------------------------------------------------------------
   ...
   this breakpoint. The stage is pinned to the top of the screen,
   horizontally centred, and sized to 40% of viewport height.
   Change ORB_MOBILE_MAX to 991 if tablets should get this too.
   ------------------------------------------------------------ */
const ORB_MOBILE_MAX = 767; // px
const ORB_MOBILE_VH = 40; // % of viewport height the orb occupies
const ORB_MOBILE_TOP = 0; // px from the top of the viewport
...
function applyMobileOrbLayout() {
  if (!orbStage) return;
  // Square stage: height drives the orb's rendered size, and a square
  // keeps the sphere round instead of stretched by the canvas aspect.
  const size = Math.round(window.innerHeight * (ORB_MOBILE_VH / 100));
  orbStage.style.position = "fixed";
  orbStage.style.top = ORB_MOBILE_TOP + "px";
  orbStage.style.left = "50%";
  orbStage.style.width = size + "px";
  orbStage.style.height = size + "px";
  orbStage.style.margin = "0";
  orbStage.style.pointerEvents = "none";
  // No scale, no scroll offset — only the centring translate.
