...
/* ------------------------------------------------------------
   ...
   a different rule: the orb is parked at the top of the hero at
   full viewport width, and it fades out the moment you scroll off
   the hero. Scroll back to the top and it fades in again.

   All four numbers below are the tuning knobs.
   ------------------------------------------------------------ */
...
const MOBILE_ORB_TOP_VH = 14; // gap from viewport top to top of the orb
...
/** Y offset (in vh, measured from viewport center) that puts the top of the
 *  circle MOBILE_ORB_TOP_VH below the top of the screen. Recomputed every
 *  frame because mobile browsers change innerHeight as the URL bar hides. */
function mobileOrbY() {
  const vh = window.innerHeight;
  const diameter = window.innerWidth * MOBILE_ORB_WIDTH;
  const centerFromTop = (vh * MOBILE_ORB_TOP_VH) / 100 + diameter / 2;
...
function rafLoop() {
  ...
  if (isMobile()) {
    // Pinned at the top, full width, no journey. Position is set directly
    // rather than lerped — there's nothing to glide toward.
    ...
    orbStage.style.transform = `translate(0vw, ${_orbY}vh) scale(1)`;
