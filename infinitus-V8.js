...
/* ============================================================
   PART 5 - STARFIELD
   Paints the #starfield canvas with a simple, lightweight field
   of small dots that drift slowly across the screen and wrap
   around the edges. Two size tiers (dim far dots, slightly
   brighter near dots) keep a touch of depth without any of the
   galaxy/nebula/cluster set-dressing.

   Star palette stays cool (white-blue) so it contrasts with the
   orb's warm-purple body and champagne highlight. Density is
   tuned to feel atmospheric, not loud — the orb is still the
   focal point.

   Positions + motion vectors are generated once on load and on
   debounced resize; the drift itself runs on requestAnimationFrame.
   ============================================================ */
const STAR_COUNT = 220;
let starfieldStars = [];
let starfieldCtx = null;
let starfieldW = 0, starfieldH = 0, starfieldDpr = 1;
let starfieldRAF = null;

function buildStarfieldStars(w, h) {
  const stars = [];
  for (let i = 0; i < STAR_COUNT; i++) {
    const isNear = Math.random() < 0.22; // ~22% are the slightly bigger/brighter tier
    const angle = Math.random() * Math.PI * 2;
    const speed = (isNear ? 0.012 : 0.005) + Math.random() * (isNear ? 0.02 : 0.01);
    stars.push({
      x: Math.random() * w,
      y: Math.random() * h,
      r: isNear ? 1.0 + Math.random() * 0.8 : 0.5 + Math.random() * 0.5,
      a: isNear ? 0.55 + Math.random() * 0.35 : 0.2 + Math.random() * 0.35,
...
function setupStarfield() {
  ...
  starfieldH = h;
  starfieldDpr = dpr;
  starfieldStars = buildStarfieldStars(w, h);
}
...
function drawStarfieldFrame() {
  ...
  for (let i = 0; i < starfieldStars.length; i++) {
    ...
    // Drift, wrapping around edges with a small margin so dots don't
    // pop in/out right at the viewport boundary.
    s.x += s.vx;
    s.y += s.vy;
    if (s.x < -2) s.x = w + 2;
    if (s.x > w + 2) s.x = -2;
  ...
  starfieldRAF = requestAnimationFrame(drawStarfieldFrame);
}

setupStarfield();
if (starfieldCtx) drawStarfieldFrame();

let _starResizeTimer;
window.addEventListener('resize', () => {
  clearTimeout(_starResizeTimer);
  _starResizeTimer = setTimeout(() => {
