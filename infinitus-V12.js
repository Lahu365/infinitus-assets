...
class Orb {
  constructor(stage, segments = 100) {
    ...
    // Default to the LIGHT matcap pair (rim fades to white, not black)
    // so the orb reads correctly on a white stage out of the box.
    // Only falls back to the dark/black-edged pair if the page has
    // explicitly opted into dark mode via data-theme="dark".
    const initialTheme =
...
(function () {
  ...
  function currentStarRGB() {
    return document.documentElement.getAttribute('data-theme') === 'dark'
      ...
      : '30, 26, 60';     // deep indigo dot — visible on light bg (default)
  }
...
(function () {
  ...
  toggle.addEventListener('click', function () {
    ...
    try { localStorage.setItem('infinitus-theme', goingDark ? 'dark' : 'light'); } catch (e) {}
  });
})();
