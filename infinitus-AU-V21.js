...
class Orb {
  ...
  /** Public resize. Call this after changing the stage element's box size
   *  in JS (the mobile sizing below does exactly that) — the window resize
   *  listener alone won't catch a size change we made ourselves. */
  resize() {
  ...
  /** Stop the three render passes while the orb is invisible. On mobile the
   *  orb is hidden for most of the page, and a paused GPU there is the
   *  difference between a warm phone and a cool one. */
  setPaused(v) {
    this._paused = !!v;
  }
...
/* ------------------------------------------------------------
   MOBILE ORB BEHAVIOUR
   The desktop journey (flying across the page, shrinking into the
   footer mark) has nowhere to go on a 390px screen, so mobile gets
   a different rule: the orb sits top-centre at full viewport width,
   idles with a slow organic float, and fades out the moment you
   scroll off the hero. Scroll back to the top and it fades in again.

   The numbers below are the tuning knobs.
   ------------------------------------------------------------ */
const MOBILE_BP = 900; // same breakpoint the pillars carousel uses
const MOBILE_ORB_WIDTH = 0.92; // orb diameter as a share of viewport width
const MOBILE_ORB_TOP_REM = 2; // gap from viewport top to top of the orb
const MOBILE_FADE_START_VH = 0.05; // scroll depth where the fade starts
const MOBILE_FADE_END_VH = 0.55; // scroll depth where it is fully gone

// Idle float. Matched off the reference clip: the orb drifts about 8% of
// its own width peak-to-peak, reversing roughly every 1.6s. Amplitudes are
// a share of the orb's diameter, so the motion scales with the screen.
const MOBILE_FLOAT_X = 0.03; // horizontal sway, ±3% of diameter
const MOBILE_FLOAT_Y = 0.022; // vertical bob, ±2.2% of diameter
const MOBILE_FLOAT_SCALE = 0.02; // breath, ±2% of size
const MOBILE_FLOAT_PERIOD = 3400; // ms for one pass of the primary wave

const isMobile = () => window.innerWidth < MOBILE_BP;

// The camera is fov 60 at z 2.4, so the visible frame at the sphere's plane
// is 2 * tan(30deg) * 2.4 = 2.771 units tall. The sphere is 2 units across,
// so it fills 72.2% of a square stage. Dividing by this ratio works back
// from "how wide should the circle look" to "how big is the stage box".
const ORB_FILL = 2 / (2 * Math.tan((60 / 2) * (Math.PI / 180)) * 2.4);

// rem is read from the document rather than assumed to be 16px, so the
// 2rem gap tracks the site's base font size. Re-read on resize.
let _remPx = 16;
function readRem() {
...
/** Size the stage box so the rendered circle matches the viewport width.
 *  This resizes the actual renderer rather than CSS-scaling the canvas, so
 *  the orb stays sharp at full width. Clearing the inline width/height
 *  hands control back to the stylesheet on desktop. */
function sizeOrbStage() {
  if (!orbStage) return;
  if (isMobile()) {
    const px = Math.round((window.innerWidth * MOBILE_ORB_WIDTH) / ORB_FILL);
    orbStage.style.width = px + "px";
    orbStage.style.height = px + "px";
  } else {
    orbStage.style.width = "";
    orbStage.style.height = "";
  }
  orb.resize();
}
sizeOrbStage();

/** Resting Y offset (in vh, measured from viewport centre) that puts the top
 *  of the circle MOBILE_ORB_TOP_REM below the top of the screen. Recomputed
 *  every frame because mobile browsers change innerHeight as the URL bar
 *  hides — the gap stays 2rem either way. */
function mobileOrbY() {
  const vh = window.innerHeight;
  const diameter = window.innerWidth * MOBILE_ORB_WIDTH;
  const centerFromTop = MOBILE_ORB_TOP_REM * _remPx + diameter / 2;
  return ((centerFromTop - vh / 2) / vh) * 100;
}

/** Idle float. Two sine waves per axis at an off ratio (0.61, 0.47) so the
 *  loop never lands on itself and the drift reads as organic rather than a
 *  metronome. Returns offsets already converted to vw / vh / scale factor. */
function mobileFloat(now) {
  const w = (now / MOBILE_FLOAT_PERIOD) * Math.PI * 2;
  const diameter = window.innerWidth * MOBILE_ORB_WIDTH;

  const xPx =
    diameter *
    MOBILE_FLOAT_X *
    (Math.sin(w) * 0.7 + Math.sin(w * 0.61 + 1.7) * 0.3);
  const yPx =
    diameter *
    MOBILE_FLOAT_Y *
    (Math.cos(w * 0.83) * 0.7 + Math.sin(w * 0.47 + 0.9) * 0.3);

  return {
    x: (xPx / window.innerWidth) * 100,
    y: (yPx / window.innerHeight) * 100,
    s: 1 + Math.sin(w * 0.53 + 2.1) * MOBILE_FLOAT_SCALE,
  };
}

// Toggle visibility + GPU work only on change, not every frame.
let _orbHidden = false;
function setOrbHidden(hidden) {
  if (hidden === _orbHidden) return;
  _orbHidden = hidden;
  orbStage.style.visibility = hidden ? "hidden" : "visible";
  orb.setPaused(hidden);
}
...
function rafLoop() {
  ...
  // 2 + 3. Update the orb. Mobile is pinned and fades; desktop flies.
  const progress = clamp01(currentScrollY / _docH);

  if (isMobile()) {
    // Pinned top-centre, full width, no journey — but never dead still.
    // Position is set directly rather than lerped; the float IS the motion,
    // so lerping on top of it would only smear the wave.
    const fadeStart = window.innerHeight * MOBILE_FADE_START_VH;
    const fadeEnd = window.innerHeight * MOBILE_FADE_END_VH;
    const fade = clamp01((currentScrollY - fadeStart) / (fadeEnd - fadeStart));

    const f = mobileFloat(performance.now());
    _orbX = f.x;
    _orbY = mobileOrbY() + f.y;
    _orbS = f.s;
    // Opacity still eases, so leaving and returning to the top reads as a
    // fade rather than a pop. Slightly snappier than the desktop drift.
    _orbO = lerp(_orbO, 1 - smoothstep(fade), _LERP * 2);

    orbStage.style.transform = `translate(${_orbX}vw, ${_orbY}vh) scale(${_orbS})`;
    orbStage.style.opacity = _orbO;
    setOrbHidden(_orbO < 0.01);
  } else {
    ...
    // Lerp displayed values toward target — this is what makes it smooth
...
// Initialise lerp targets to starting position before first frame, so the
// orb doesn't visibly slide in from the wrong place on load.
if (isMobile()) {
  _orbX = 0;
  _orbY = mobileOrbY();
  _orbS = 1;
  _orbO = window.scrollY > window.innerHeight * MOBILE_FADE_END_VH ? 0 : 1;
} else {
...
// Resize: re-measure everything and re-size the orb stage. Debounced,
// because on mobile every URL-bar show/hide fires a resize and rebuilding
// the renderer buffers on each one would stutter the scroll.
let _resizeT;
...
window.addEventListener("resize", () => {
  ...
  _resizeT = setTimeout(() => {
    // Width is what drives the orb's size, so ignore height-only resizes.
