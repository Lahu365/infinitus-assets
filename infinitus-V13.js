...
const BRAND = {
  ...
  // Softened stand-in for pure black at the orb's rim/shadow. Still
  // reads as "dark edge" (keeps the original moody falloff) but has
  // enough indigo in it that it doesn't look like a flat black ring,
  // and sits much closer to a white page than #010007 would.
  shadow:   '#1B1540',
};
const hexToRGB = (h) => {
...
function makeMatcap(stops, highlightColor = BRAND.lavender, edgeColor = BRAND.black) {
  const size = 512;
...
// ---- Light-theme pair --------------------------------------------
// Same idea as GALAXY/AMETHYST_DEEP above, but built for a white
// stage: the rim (the 1.00 stop, sampled at grazing/silhouette
// angles) fades to white instead of black so the orb's edge melts
// into a light page the same way the dark pair melts into a black
// one. The highlight glint is swapped to a darker indigo tone so it
// still reads as a pin-prick of contrast against the lighter body
// (a light glint would disappear against a light edge).
const GALAXY_MATCAP_LIGHT = () => makeMatcap([
  [0.00, BRAND.champagne],
  [0.18, BRAND.lavender],
  [0.42, BRAND.indigo],
  [0.66, BRAND.amethyst],
  [0.88, BRAND.orchid700],
  [1.00, '#FFFFFF'],
], BRAND.indigo300, '#FFFFFF');

const AMETHYST_DEEP_MATCAP_LIGHT = () => makeMatcap([
  [0.00, BRAND.lilac],
  [0.22, BRAND.orchid],
  [0.55, BRAND.amethyst],
  [0.84, BRAND.indigo900],
  [1.00, '#FFFFFF'],
], BRAND.indigo300, '#FFFFFF');
...
class Orb {
  constructor(stage, segments = 100) {
    ...
    // Both theme pairs are generated up front (cheap 512x512 canvas
    // draws) so switching themes is just a uniform swap, not a
    // texture rebuild - no flash/stall when the toggle is clicked.
    this._matcaps = {
      dark:  { a: GALAXY_MATCAP(),       b: AMETHYST_DEEP_MATCAP() },
      light: { a: GALAXY_MATCAP_LIGHT(), b: AMETHYST_DEEP_MATCAP_LIGHT() },
    };
    const initialTheme =
    ...
    this.renderMat = new THREE.RawShaderMaterial({
      ...
      uniforms: {
        ...
        matcapTexture:  { value: this._matcaps[initialTheme].a },
        matcapTexture2: { value: this._matcaps[initialTheme].b },
  ...
  /** Swap in the light or dark matcap pair. Cheap - just repoints the
   *  two texture uniforms, no re-render of the canvases involved. */
  setTheme(theme) {
    const pair = theme === 'light' ? this._matcaps.light : this._matcaps.dark;
    this.renderMat.uniforms.matcapTexture.value = pair.a;
    this.renderMat.uniforms.matcapTexture2.value = pair.b;
  }
  ...
  _bindEvents() {
    ...
    // Same data-theme contract PART 6 (theme toggle) and the starfield
    // IIFE use: only fires on an actual attribute change, not per frame.
...
(function () {
  ...
  // Theme-aware star color: dark theme stars are cool white/blue,
  // light theme stars need to be a dark tone to stay visible on a
  // light background. MutationObserver only fires on an actual
  // data-theme change, not every frame.
  function currentStarRGB() {
    return document.documentElement.getAttribute('data-theme') === 'light'
      ? '30, 26, 60'      // deep indigo dot — visible on light bg
      : '225, 232, 255';  // cool white — visible on dark bg
  }
  let starRGB = currentStarRGB();
  new MutationObserver(() => { starRGB = currentStarRGB(); })
...
/* ============================================================
   PART 6 - THEME TOGGLE
   Flips data-theme on <html> between dark (default, no attr) and
   light. CSS variables handle the page repaint; the starfield
   IIFE above picks up the change via its own MutationObserver.
   Persisted to localStorage; the matching anti-flash script goes
   in Webflow's Custom Code > Head (see project notes).
   ============================================================ */
(function () {
  ...
  toggle.addEventListener('click', function () {
    ...
    try { localStorage.setItem('infinitus-theme', goingLight ? 'light' : 'dark'); } catch (e) {}
  });
})();
