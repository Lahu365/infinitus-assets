...
let _markAbsCenter = 0;
let _pillarsAbsTop = 0;
let _pillarsHeight = 0;
...
function updatePillarsTrack() {
  ...
  // Use cached absolute positions — no getBoundingClientRect in hot path
  var sectionTop    = _pillarsAbsTop - window.scrollY;
...
// Cache layout once the page is fully painted, then on resize.
// Double-RAF + 300ms ensures Webflow fonts/images have shifted layout
// into final position before we snapshot offsetTop values.
function initLayout() {
  cacheLayout();
  updatePillarsTrack();
}
