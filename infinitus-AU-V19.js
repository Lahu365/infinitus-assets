...
class Orb {
  constructor(stage, segments = 100) {
    ...
    this._autoSpeed = 1; // raise for a faster wander, 0.6 for a lazier one
    this._autoTarget = new THREE.Vector2(-2, -2);
  ...
  _loop() {
    ...
    // Virtual pointer (mobile). Two out-of-phase sine pairs give a
    // wandering path that never repeats on a short cycle, so it doesn't
    // read as a loop. Amplitude is capped at 0.72 of the silhouette
    // radius (0.397 x stage height at camera z 2.4, 60deg FOV) so the
    // raycast always lands on the sphere and the ripple stays alive.
    if (this._autoPointer) {
      const t = this.clock.getElapsedTime() * this._autoSpeed;
      if (this.clock.getElapsedTime() - this._lastTouchT > 2) {
        const r = rect.height * 0.397;
        const ax = Math.cos(t * 0.42) * 0.5 + Math.cos(t * 0.17 + 1.3) * 0.22;
        const ay = Math.sin(t * 0.31) * 0.5 + Math.sin(t * 0.53 + 0.7) * 0.18;
        this._autoTarget.set(
          rect.left + rect.width / 2 + ax * r,
          rect.top + rect.height / 2 + ay * r,
        );
        // Lerped, not snapped, so handing back from a real touch glides.
        this.mouse.lerp(this._autoTarget, 0.1);
      }
    }
    this.ndc.set(
